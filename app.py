...
st.markdown("""Este aplicativo corta um áudio em múltiplos trechos com base em pontos onde o volume é próximo de 0dB nos dois canais.""")
...
with col1: min_intervalo = st.number_input("Intervalo mínimo (min)", 0.5, 10.0, 2.0)
with col2: max_intervalo = st.number_input("Intervalo máximo (min)", 0.5, 10.0, 4.0)
...
def find_zero_crossings(_audio_segment):
    ...
    for i in range(frame_count):
        ...
        if abs(left) < 100 and abs(right) < 100: zero_points.append(i * 1000 / _audio_segment.frame_rate)
    ...
    if st.button("Processar Áudio"):
        try:
            ...
            if not zero_points:
                ...
                possiveis_intervalos = list(set(random.randint(min_ms, max_ms) for _ in range(num_cortes * 2)))
                ...
                for intervalo in intervalos:
                    ...
                    if ponto_offset < total_duration: pontos_corte.append(ponto_offset)
                    else: break
                ...
                for file in os.listdir(output_dir): st.download_button(label=file, file_name=file, data=open(os.path.join(output_dir, file), "rb"), mime="audio/mp3")
